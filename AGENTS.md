...
- 模块结构：
  ...
  - `modules/home/default.nix` — 单用户 Home Manager 入口（NixOS 与 nix-darwin 共用），orchestrate 共享 Home Manager 模块；`home.username` / `home.homeDirectory` 由 home-manager OS 模块自动从 `users.users.zine` 派生
  - `modules/home/common.nix` — Home Manager 通用基础配置
  - `modules/home/packages/<category>.nix` — 集中包清单（安装侧）：声明安装哪些软件（`programs.<name>.enable` 或 `home.packages`），按 `tools`/`shell`/`dev`/`terminal`/`misc`/`gui`/`desktop` 分类，`gui` 与 `desktop` 通过 `extraLibs.linuxOnly` 仅 Linux 生效；**不在这里写配置**
  ...
  - `lib/` — 可复用 Nix 函数（`mkSystem.nix`、`niri-config.nix`、`storeLinks.nix`、`nix-settings.nix`、`nixpaks-*.nix`、`scanPaths.nix`，含平台门控 `linuxOnly`）
  - `lib/storeLinks.nix` — 统一封装 in-store / out-of-store 链接策略，供 `xdg.configFile` 使用
  - `pkgs/` — 自定义/外部包定义（如 `dsh.nix`、`jj-bond.nix`、`orca.nix`），由 `modules/home/packages/` 或 `modules/nixos/` 通过 `pkgs.callPackage ../../../pkgs/<name>.nix { }` 引用
  - `vars/default.nix` — 共享变量（`git` 身份）与按 host 组织的变量（`hosts.<hostname>.hostname`、`hosts.<hostname>.hardware` 等）
- 新增 program 时：在 `modules/home/packages/<category>.nix` 中添加安装声明（`programs.<name>.enable` 或 `home.packages` 条目；自定义包定义放根 `pkgs/`），并按用途在对应类别下创建 `modules/home/programs/<category>/<name>/default.nix` 存放纯配置；目录创建后各层 `default.nix` 会自动扫描导入，无需手动注册。若现有类别都不合适，可新增类别目录并配一个调用 `extraLibs.scanPaths` 的 `default.nix`。
...
- 每个 `modules/home/programs/<category>/<name>/default.nix` 只管理该程序的配置文件（安装统一由 `modules/home/packages/<category>.nix` 声明）；`modules/home/programs/default.nix` 与 `programs/<category>/default.nix` 通过 `lib/scanPaths.nix` 逐层自动导入。
