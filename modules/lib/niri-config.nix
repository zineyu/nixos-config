...
# every `include "dms/<file>.kdl"` found in dotfiles/niri/config.kdl.
#
# This module scans the dms/ directory automatically, so adding a new
# `include "dms/foo.kdl"` only requires creating dotfiles/niri/dms/foo.kdl.
{ niriDir, lib }:

let
  inherit (builtins) readDir readFile attrNames filter;
