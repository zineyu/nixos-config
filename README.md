...
| `hosts/<hostname>/default.nix` | 该机器的 NixOS 模块，导入 `configuration.nix`（Home Manager 由 `lib/mkSystem.nix` 接线） |
...
| `home/<hostname>.nix` | 每台机器的 Home Manager 组织文件：显式导入共享定义层与裸包捆绑，并逐个启用 `zine.programs.<name>` |
| `home/common.nix` | Home Manager 通用基础配置 |
| `home/ssh.nix` | 用 sops-nix 解密并生成 SSH alias 配置（如 `aliyun-01`） |
| `modules/home/packages/` | 集中包清单（安装侧）：`zine.programs.<name>.enable` options 定义层与 opt-in 裸包捆绑（`tools.nix` / `dev-tools.nix` / `gui-extras.nix` / `desktop.nix`） |
