...
| 路径 | 说明 |
|------|------|
| `flake.nix` | Flake 入口，声明 inputs、formatter、checks、devShells、`packages.nix-conf`、`nixosConfigurations` 和 `deploy` |
...
- **新增 host**：在 `hosts/default.nix` 添加 `hostname = "system";` 映射，并创建 `hosts/<hostname>/default.nix` 和 `hosts/<hostname>/configuration.nix`。如为服务器，通常在 `configuration.nix` 中导入 `modules/nixos/server`。
