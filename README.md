...
开发环境使用 [devenv](https://devenv.sh/)，通过 `flake.nix` 中的 `devShells` 定义。进入开发环境：
...
开发环境包含 `nixfmt`、`deadnix`、`statix`、`sops`、`ssh-to-age`、`just` 等工具。

## 目录结构

| 路径 | 说明 |
|------|------|
| `flake.nix` | Flake 入口，声明 inputs、formatter、devShells 和 `nixosConfigurations` |
| `hosts/default.nix` | 主机注册表，显式映射 `hostname -> system` |
| `hosts/<hostname>/default.nix` | 该机器的 NixOS 模块，导入 `configuration.nix` 并声明 `home-manager.users.<username>` |
...
| `modules/home/tools.nix` | 共享用户工具包（如 `ripgrep`、`fzf`、`just` 等） |
| `modules/home/shell/` | 用户 shell 与启动文件（fish、bash、starship） |
| `modules/home/desktop/` | 用户级桌面环境组件（niri、DankMaterialShell、认证代理等） |
| `modules/home/programs/<name>/` | 每个用户程序一个目录，原生配置文件与模块共置 |
| `modules/nixos/` | 系统级 NixOS 模块（音频、网络、Nix 设置、用户账户等） |
| `lib/` | 可复用 Nix 函数（如 `mkSystem.nix`、`niri-config.nix`、`storeLinks.nix`） |
| `docs/adr/` | 架构决策记录（ADRs） |
| `secrets/` | `sops-nix` 加密的 secrets |

## 新增内容

- **新增 program**：在 `modules/home/programs/<name>/default.nix` 创建模块，并在 `modules/home/programs/default.nix` 中注册。
- **新增 host**：在 `hosts/default.nix` 添加 `hostname = "system";` 映射，并创建 `hosts/<hostname>/default.nix` 和 `hosts/<hostname>/configuration.nix`。
- **新增 NixOS 模块**：在 `modules/nixos/` 下添加，并导入到对应主机的 `configuration.nix` 中。
