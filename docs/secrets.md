...
- `secrets/aliyun-01.yaml`：加密保存 `luogo_checkin` 服务所需的环境变量（由 `hosts/aliyun-01/configuration.nix` 直接引用），以及 vaultwarden 的 `ADMIN_TOKEN`（明文，由 `modules/nixos/server/vaultwarden.nix` 作为 environmentFile 注入，可用 `sops -d` 取回）。
