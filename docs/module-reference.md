...
| `system.nix` | 引导加载器、内核、最小系统包。 | `systemd-boot`、`linuxPackages_latest`、fish、vim、wget |
...
| `steam.nix` | 启用 Steam、gamescope 与 32 位图形驱动。 | `programs.steam`、`hardware.graphics.enable32Bit` |
...
|------|---------|---------|
| `default.nix` | 通过 `scanPaths` 自动导入同级 `.nix` 文件。 | `extraLibs.scanPaths` |
| `docker.nix` | 启用 Docker 守护进程并开机自启。 | `virtualisation.docker` |
...
| `tools.nix` | 通用 CLI 工具（搜索、构建、获取等）。 | `fzf`、`ripgrep`、`eza`、`btop`、`just`、`wl-clipboard` |
...
| `bash/default.nix` | 最小化 Bash 配置。 | `programs.bash`、基础别名 |
...
| File | Purpose | Notable |
|------|---------|---------|
| `default.nix` | 导入 niri、DankMaterialShell 与桌面环境变量模块。 | — |
...
| `devtools/` | 开发工具包（编译器、格式化工具、实用程序）。 | `clang`、`go`、`maven`、`uv`、`github-cli` 等 |
...
| File | Purpose | Notable |
|------|---------|---------|
| `vars/default.nix` | 共享变量（Git 身份、主机名、硬件总线 ID）。 | `git.name`、`linux.hostname`、`linux.hardware.*` |
...
- **`lib/storeLinks.nix`** — 被 `modules/home/programs/fontconfig`、`modules/home/programs/kitty`、`modules/home/programs/neovim` 使用，显式选择 in-store 或 out-of-store 软链接策略。
